...
private const val MAX_DECOMPRESSED_PAYLOAD_BYTES = 4 * 1024 * 1024
...
fun parseDoubaoSpeechFrame(frame: ByteArray): DoubaoSpeechFrame {
    if (frame.size < 8) throw IllegalArgumentException("doubao frame too short")
    ...
    return when (messageType) {
        VOLC_MESSAGE_FULL_SERVER_RESPONSE, VOLC_MESSAGE_SERVER_ACK -> {
            ...
            DoubaoSpeechFrame(text = text, final = isFinal, hasTranscript = text.isNotEmpty())
        }

        VOLC_MESSAGE_SERVER_ERROR -> {
            if (payload.size < 8) throw IllegalArgumentException("doubao error frame too short")
...
private fun doubaoHeader(messageType: Int, flags: Int, serialization: Int, compression: Int): ByteArray =
    byteArrayOf(
...
private fun readDoubaoPayload(payload: ByteArray, compression: Int): ByteArray {
    if (payload.size < 4) throw IllegalArgumentException("doubao payload missing size")
...
private fun extractDoubaoText(body: ByteArray): String = try {
...
private fun extractDoubaoResultText(value: Any?): String = when (value) {
    null, JSONObject.NULL -> ""
    is String -> value
    is JSONArray -> buildString {
        for (index in 0 until value.length()) append(extractDoubaoResultText(value.opt(index)))
    }
    is JSONObject -> {
        ...
        if (text.isNotEmpty()) {
            ...
            if (utterances == null) "" else buildString {
                for (index in 0 until utterances.length()) append(extractDoubaoResultText(utterances.opt(index)))
            }
        }
    }
    else -> ""
...
private fun extractDoubaoErrorMessage(body: ByteArray): String = try {
    val root = JSONObject(body.decodeToString())
...
private fun gunzip(raw: ByteArray): ByteArray {
    ...
    GZIPInputStream(ByteArrayInputStream(raw)).use { input ->
        ...
        while (true) {
            ...
            if (read < 0) break
            if (output.size() + read > MAX_DECOMPRESSED_PAYLOAD_BYTES) {
                throw IllegalArgumentException("doubao decompressed payload is too large")
            }
...
fun uint32Bytes(value: Int): ByteArray = int32Bytes(value)
