...
func (r *Reporter) sendHubPing(conn *websocket.Conn) error {
	...
	if err := r.writeJSON(conn, "->", envelope{
		RequestID: requestID,
		Type:      rp.RegistryEnvelopeTypeRequest,
		Method:    rp.RegistryMethodHubPing,
...
func (r *Reporter) handshake(conn *websocket.Conn) error {
	if err := r.writeJSON(conn, "->", envelope{
		...
		Payload: rp.MustRaw(map[string]any{
			...
			"role":            string(rp.RegistryRoleHub),
			"hubId":           r.cfg.HubID,
			"token":           r.cfg.Token,
		}),
	}); err != nil {
...
func (r *Reporter) handleLocalReadConnectInit(conn *websocket.Conn, state *localReadConnectionState, in envelope) {
	...
	if strings.TrimSpace(payload.Role) != string(rp.RegistryRoleLocalRead) {
	...
	_ = r.writeJSON(conn, "->", envelope{
		...
		Payload: rp.MustRaw(rp.ConnectInitResponsePayload{
			...
			Principal: rp.ConnectPrincipal{
				Role:  string(rp.RegistryRoleLocalRead),
				HubID: r.cfg.HubID,
			},
...
func localReadMethodAllowed(method string) bool {
	return rp.RegistryLocalReadMethodAllowed(method)
}
...
func (r *Reporter) replySession(conn *websocket.Conn, req envelope) {
	...
	if err := r.writeJSON(conn, "->", envelope{
		...
		registryLogger(projectID).Warn("session response write failed requestId=%d method=%s projectId=%s err=%v", req.RequestID, req.Method, projectID, err)
		return
	}
}
...
func (r *Reporter) writeVerboseEnvelope(direction string, v any) {
	log := registryLogger("")
	if !log.VerboseEnabled() {
		return
	}
	env, ok := envelopeFromValue(v)
	if !ok {
		return
	}
	dir := verboseEnvelopeDirection(direction)
	registryLogger(env.ProjectID).Verbose("envelope dir=%s type=%s requestId=%d method=%s projectId=%s hubId=%s", dir, env.Type, env.RequestID, env.Method, env.ProjectID, env.HubID)
}
...
func verboseEnvelopeDirection(direction string) string {
