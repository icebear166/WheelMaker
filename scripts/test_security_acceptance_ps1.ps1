...
foreach ($source in @($powershell, $shell)) {
    ...
    Assert-Contains 'acceptance entry' $source 'configure-public-url --config "$gateway_config_candidate"'
    Assert-Contains 'acceptance entry' $source '$HOME/.wheelmaker/gateway/config.json'
    Assert-Contains 'acceptance entry' $source 'systemctl --user'
    Assert-Contains 'acceptance entry' $source 'gateway/sites'
