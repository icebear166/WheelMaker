...
Add assertions that `session.status` and `session.compact` are project-scoped `RegistryRouteSessionForward` methods, protocol version remains `2.6`, the default Codex factory advertises both actions, and a non-Codex provider advertises neither.
...
Add both constants and descriptors. Extend `ACPFactory` with a cloned `sessionActions map[ACPProvider]SessionActionSupport`, register Codex support beside its built-in creator, and expose `RegisterSessionActions`/`SessionActions`. Keep Go and TypeScript protocol versions at `2.6`.
...
Define action capability, status context/limit/account response, compact response, and operation payload interfaces. Add `SessionStatus`/`SessionCompact` methods and keep `RegistryProtocolVersion` at `2.6`. Repository methods use a 30-second request timeout and do not expose unknown provider payload fields.
...
**Files:**
- Modify if needed: `docs/scope/2026-07-14-codex-session-actions.md`
- Track: `docs/plans/2026-07-14-codex-session-actions/plan-codex-session-actions.md`
