...
- [x] **Step 1: Run full server verification**
  - Run `go test ./server/internal/hub/... ./server/internal/hubconfig/... ./server/internal/protocol/...`.
  - Result: MCP-related packages and repeated individual reporter tests pass. The aggregate Hub run remains subject to existing Windows `TempDir RemoveAll cleanup` races in unrelated reporter tests.
- [x] **Step 2: Run full affected web verification**
  - Run `npm test -- --runInBand` from `app` and `npm run tsc:web`.
  - Result: MCP-focused tests and `npm run tsc:web` pass. The aggregate Jest run still has pre-existing assertions for unrelated brand SVG and responsive/settings CSS fixtures; no such files are changed by this feature.
