...
- 选中态显示 persisted `mode`；运行状态仍由现有状态点和 Running/Stopped/Failed 文案表达。
...
4. manager 以相同 host、port 和 key 启动目标隐藏入口并等待健康检查。
...
- 现有 `cc-flicker` provider 不修改 endpoint、agent ID、key 语义或配置目录。
