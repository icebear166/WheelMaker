...
- 每个 Hub 独立保存 OAuth 凭据并独立查询；Qwen 不跨 Hub 合并账号，每个配置了 Token Plan Key 的 Hub 展示独立行，行上的登录、退出、快照和趋势操作都直接作用于该 Hub。
...
- Hub 支持自动续期。续期失败后保留登录失败状态并要求重新登录；退出登录只清除 OAuth 凭据，不清除 Qwen API Key。
...
1. Hub 读取现有 Qwen API Key，只有检测到非空且以 `sk-sp-` 开头的 Key 时，向 HubState 暴露该 Hub 的 Qwen 占位 Provider；普通 `sk-` 或 clear 会产生显式移除结果。
2. 用户在 Limits Monitor 点击目标 Hub 的 Qwen 占位行，打开详情；若当前 Hub 没有有效 OAuth 凭据，详情显示登录操作。
3. 桌面/Android 原生运行时打开百炼 Console 登录窗口。登录完成后，凭据通过 Hub-scoped 的受保护写入路径保存到该 Hub；不得把凭据放入常规状态广播。
4. Hub 使用 OAuth 凭据请求当前 Credits 快照，发布该 Hub 专属的 Qwen Provider；Qwen 占位行替换为额度行。
5. 详情打开或手动刷新时，前端向该行对应的 Hub 请求本地历史趋势；Hub 返回现有历史协议的归一化样本和查询时间。
...
- Qwen Provider 的 `LocalID` 在 Hub 内保持稳定；每个 Hub 只产生一个官方 Token Plan 账号，前端不得把不同 Hub 的 Qwen 账号合并成同一账号。
...
- 当前快照继续通过现有 `hub.state.get` / `hub.state.refresh` 和 `tokenStats` section 传播，Registry 只做权限校验和转发；Qwen 的账号身份必须保持 Hub scope，不能依靠全局 identity 合并。
...
- 扩展 `app/web/src/usage` 的 Provider 类型、排序、解析和状态逻辑，使 `qwen` 的 `hidden → placeholder → ready → stale/error → placeholder/hidden` 状态迁移可表达；Qwen 的配置移除必须删除旧行，不能显示为旧成功数据。
- 在现有 Limits Monitor 的紧凑行和详情路径中增加 Qwen；每个 Hub 独立一行，紧凑行按有限窗口中更低剩余比例摘要，详情显示双窗口、订阅/用量包和本地历史趋势。
...
- Qwen 不沿用跨 Hub 的账号聚合；Hub pill 作为来源标识，每一行本身就是登录、退出、快照刷新和趋势查询的唯一目标，不把多个个人订阅合计为一个 Credits 总额。

### 原生登录桥接

- 桌面和 Android 原生运行时增加 Qwen/Bailian 登录能力：打开受控登录窗口、等待登录完成、返回受保护的登录结果、处理用户关闭和超时。与 DeepSeek 的登录窗口结构只能作为宿主生命周期参考，不得假设百炼使用相同的 localStorage、回调或 Token 形状。
- 登录窗口必须限制导航/脚本能力到百炼 Console 信任范围，并清理窗口生命周期；不得把浏览器 Cookie 写入普通 Web storage 或工作区文件。
- 登录成功后的凭据写入应直接关联发起登录的 Hub；多个 Hub 同时存在时不得把凭据写到全局用户配置。
...
5. 扩展协议类型、Web usage store、Monitor Provider 行和详情 UI，并复用本地历史趋势。
...
- OAuth 成功后，每个目标 Hub 的 Qwen 行展示官方 5 小时/7 天 Credits 数据；窗口正确表达 `limited/unlimited/unavailable`，紧凑行只在有限窗口中选择更低剩余比例，详情展示两个窗口绝对 Credits。
...
- 多 Hub 的 OAuth、快照、趋势和错误彼此隔离，每个 Hub 一行且每次操作目标唯一，不错误合计个人订阅 Credits。
...
- `app/web/src/usage`、Monitor/Workspace wiring：Provider 状态、占位行、详情、趋势和错误展示。
- 对应 Go/Jest/类型检查测试及脱敏的接口 fixture；不写入真实凭据。

本次已进入实现阶段；不更新 `docs/wiki`，真实百炼账号登录、OAuth 续期和当前快照仍遵守上述外部接口验证门槛；Qwen 趋势不再依赖官方趋势能力。
