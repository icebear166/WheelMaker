...
本规格修订 `docs/scope/2026-07-13-system-security-baseline.md` 中“Native Bridge 不得获得语音密钥原文”和“Android 语音必须由后端执行”的目标状态：只有经过认证、声明为 Android 的客户端可读取 Volcengine ASR Key，且这一限制采用简单客户端标识，不提供密码学设备证明。
