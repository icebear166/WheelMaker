...
- **Q：清理保留策略？** A：只删除 `stable.json` 和 `releases.json` 都不引用的 `v1.x` 目录（孤儿目录）；两个元数据文件都不修改，被引用的历史版本一律保留。`staging/`、`debug-web/` 本就有自动清理，不在范围内。
...
- **release server**（`server/internal/releaseserver`）：新增两个 `/api/` 端点，复用现有 Bearer token 鉴权。引用集 = `stable.json` 引用的全部版本（`stable.Version` 以及 Desktop/Android 指针版本——指针允许指向历史版本目录）∪ `releases.json` 全部 entry 的 version。storage 与 prune 都和 commit 共用同一把互斥锁（`commitMu`），避免把 commit 过程中尚未写入 `releases.json` 的新版本目录误判为孤儿。
  - `GET /api/storage` → `{totalBytes, reclaimableBytes, orphanCount}`：遍历 `public/releases/` 下的版本目录，统计总占用与不属于引用集的目录占用及数量。
  - `POST /api/prune` → `{ok, removedCount}`：仅删除 `public/releases/` 下匹配 `v1.x` 命名且不属于引用集的目录。`stable.json` 缺失或损坏、`releases.json` 读取失败时拒绝删除并返回错误。
...
- 清理按钮二次确认后删除所有无引用版本目录；`stable.json`、`releases.json`、被引用版本目录（含 `stable.json` Desktop/Android 指针指向的历史版本）、`staging/`、`debug-web/` 保持不变；清理后占用数字自动刷新。
...
- release server：storage 统计（含孤儿/引用目录混合）、prune 只删无引用目录、stable 缺失时拒绝、未授权拒绝，合并到现有 `server/internal/releaseserver` 测试文件。
