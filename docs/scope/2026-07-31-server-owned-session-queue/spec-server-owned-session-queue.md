...
- Queue 非空（包括 failed 后暂停）时，Session 不得被闲置回收；所有 App 断开后 Hub 仍继续调度，直到 queue 为空或失败暂停。
- Prompt 与 compact 共用一个 FIFO。Goal、status、fork 等 Session action 不进入 queue；其他正在运行的 Session execution 会阻止 queue 启动下一项。
- Queue snapshot 包含 `generation`、单调递增的 `revision`、`paused`、`activeItem` 和 `waitingItems`。reload 或 Hub 重启产生新的 generation，App 只在同一 generation 内比较 revision。
- Item 状态包括 `queued`、`running`、`cancelling`、`steering` 和 `failed`。`completed`、`cancelled`、`steered` 完成终态通知并进入正式 turn/operation 表达后，从 live queue 移除；failed item 保留为 active failure。
...
- 使用统一的 `session.queue` request method，action 仅包含 `enqueue`、`cancel`、`prioritize`、`steer`、`retry`。
...
- Queue 的 enqueue、排序、状态变化和恢复操作复用 `session.updated` 推送完整 Session/queue snapshot；操作响应也返回最新 snapshot。`session.list` 只返回 queue generation/revision、paused、active kind 和 waiting count 摘要，不返回 item blocks。
- Hub 按接收顺序串行执行所有 queue mutation，并在每次可观察变化后递增 revision、发布 `session.updated`。
- Active item 正常完成后自动执行下一项。Active prompt 主动 Stop 后标记 cancelled、移出 queue并自动继续。
- Active item 执行失败后保留为 failed 并暂停 queue，不自动执行后续 waiting items。`retry` 使用原 item 重试；对 failed item 执行 `cancel` 会移除失败项并恢复调度。
...
- `session.reload` 仅在没有 active execution 时允许，并直接清空 waiting/failed queue 和幂等状态、创建新的 queue generation。
...
- Queue 不接管附件文件清理。Waiting/failed item 被 cancel 或 Hub 重启遗留的未发送附件保留到 Session archive/delete 时由现有目录清理流程删除。
...
Queue scheduler 必须从 prompt 和 compact 执行链得到明确的成功、取消或失败结果，再原子完成 active item、决定暂停或继续。现有 steer fallback 优先队列并入 Session queue 调度，避免同时存在两套 prompt waiting 所有者。
...
4. 若 Session 没有其他 execution、queue 未暂停且没有 active item，Hub 原子地把队首提升为 running，并在后台执行。
5. 正常完成或 prompt cancelled 时，Hub 发布终态、移除 active item并继续队首；失败时保留 failed item并暂停。
...
- App 全部断开时 queue 继续执行；重连后 `session.read` 恢复当前 active/waiting/failed 投影。
- Hub 重启后 queue 为空；历史 turns 和 Session SQLite 数据保持可读。
- Queue 非空时 Session 不会被闲置回收；queue 清空后恢复现有 suspend/persist 行为。
- Active item 失败后 queue 暂停；retry 重试原 item；cancel failed item 后继续下一项。
...
- Hub queue 状态测试覆盖统一 FIFO、并发 enqueue 幂等、ID 冲突、prioritize、waiting cancel、失败暂停、retry、failed cancel 恢复、revision/generation 和无人在线时持续 drain。
...
- App 状态与集成测试覆盖仅消费服务端 snapshot、generation/revision 去旧、操作响应与 event 汇合、运行中附件入口、附件上传先于 enqueue，以及 queued/steering/failed/cancelling UI 操作。
