...
- **接口结构以真实抓包为唯一依据**（2026-08-01 实测，开源项目 CodexBar 交叉验证一致；脱敏 fixture 存于 [fixtures/](fixtures/)）：信封统一为 `{code, msg, data: {biz_code, biz_msg, biz_data}}`（snake_case）；顶层 `code` 或 `data.biz_code` 为 40002/40003 均视为 token 失效。废弃旧的“多 key 猜测”解析，全部按确定结构强类型解析；解析失败返回明确错误而不是静默出空数据。
