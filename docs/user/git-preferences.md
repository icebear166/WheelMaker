...
- Branch/Worktree：仅落 spec 的任务创建 feature branch，并绑定创建项目根目录 `.worktree/<branch-name>`；不落 spec 的任务和 bug 修复沿用当前分支；创建后立即运行 `node scripts/bootstrap-worktree.mjs` 初始化独立 npm 环境，禁止通过 Junction 共享 `node_modules`，同时确保 `.worktree/` 被 Git 忽略。
