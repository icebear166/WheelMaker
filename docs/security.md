...
Claude-compatible agent 的 Hub 本地 Key 是独立于 Server Data 的明确例外。每个 Hub 在自己的 `<stateDir>/db/hub-config.json` `apiKeys` section 保存 DeepSeek、Kimi Code、阿里云百炼 Token Plan、Z.AI Key 与可选的本地 MyFlickerBridge gate token；默认 `stateDir` 为 `~/.wheelmaker`。旧 `config.json` 中的 `api_keys` 只为平滑启动而兼容解析：Hub 会整体忽略其内容并记录不含 Key 值的迁移 warning，运行时绝不把它作为凭据来源。这些 Key 只供 Hub 构建 `cc-deepseek`、`cc-kimi`、`cc-qwen`、`cc-glm`、`cc-flicker` provider 和本地 Limits scanner 使用；Registry 与浏览器只接收 `configured` / `updatedAt` 脱敏状态，不接收明文，也不向其他 Hub 下发。MyFlicker 未显式设置 gate token 时使用 loopback-only 占位值 `00000000000000000000`；它不是 MyFlicker 上游凭据。配置在 Hub 启动时读取，API Key 变更需要重启 Hub。
