...
- V1 模型选择跟随 bridge `/v1/models` 动态 catalog 全量展示（非 Claude 模型带 `CLAUDE-MYFLICKER-` 前缀 id）。Opus/Fable 映射到 `CLAUDE_OPUS_5`，Sonnet/Haiku/Subagent 映射到 `CLAUDE_4_6`，WheelMaker 侧不做 effort 归一化。V2 固定适配 `@myflicker/cli` 0.3.16 的 bundle contract，其他版本（包括 0.3.15 和 0.3.16-beta.2）拒绝加载；由 MyFlicker AI SDK 按模型选择 Anthropic、OpenAI 或 Responses 上游格式，并接受 canonical ID、MyFlicker alias，以及能由 `epModelName` 唯一对应的 V1 model ID。当前 Wanqing catalog 额外支持 `deepseek-v4-pro-0813`、`deepseek-v4-flash-0731` 及其唯一 alias（`DeepSeek-V4-Flash 0731`），不强行暴露 catalog 未提供的 `glm-5.3`；不存在或冲突的模型不得静默替换。
- 内置 Flicker ACP loader 针对 0.3.16 的 ACP session class 和 config option 做精确补丁，注入 access 选项并保留上游原生 `unstable_resumeSession`；不增加 resume shim。启动参数仍为 `--approval-mode yolo --thinking-level xhigh acp`，不改变 ACP wire protocol 版本。Hub 的 `session/load` 仍按既有链路恢复并回放历史，native resume 不回放上游历史不会改变 WheelMaker 的 session/load 行为。
