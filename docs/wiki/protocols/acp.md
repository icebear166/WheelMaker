...
- `cc-flicker` 连接由 Hub 托管的本地 Flicker Bridge（`http://127.0.0.1:17999`），默认 `CLAUDE_OPUS_4_8`，走 **CLI gateway model discovery**（注入 `env.CLAUDE_CODE_ENABLE_GATEWAY_MODEL_DISCOVERY=1`，不用 `availableModels`/`enforceAvailableModels`）。Bridge 有两个互斥运行模式：V1 是 WheelMaker 当前内置的 Go MyFlicker 协议迁移实现；V2 是通过 Node 加载 npm `@myflicker/cli` AI SDK provider 的 Wanqing 出口。Hub 菜单选择并持久化模式，两个模式复用相同 endpoint、`cc-flicker` agent ID、配置目录和本地 key，同一时刻只有一个模式监听 17999。
- V1 模型选择跟随 bridge `/v1/models` 动态 catalog 全量展示（非 Claude 模型带 `CLAUDE-MYFLICKER-` 前缀 id），因此非 Claude 模型也可暴露 effort 选项。Opus/Fable 映射到 `CLAUDE_OPUS_4_8`，Sonnet/Haiku/Subagent 映射到 `CLAUDE_4_6`，WheelMaker 侧不做 effort 归一化。V2 由 MyFlicker AI SDK 按模型选择 Anthropic、OpenAI 或 Responses 上游格式，并接受 canonical ID、MyFlicker alias，以及能由 `epModelName` 唯一对应的 V1 model ID；不存在或冲突的模型不得静默替换。
- Hub 配置 `api_keys.flicker` 后拉起当前选中的 bridge 子进程，并把该配置值原样复用为 bridge 本地门禁与 `cc-flicker` 的本地认证值，不额外生成随机 key。MyFlicker 的设备 token/SSO 凭据仍由 bridge 自己持有，WheelMaker 不读取或传递它们。运行中切换会重启 bridge 但不主动删除 `cc-flicker` Session；目标模式失败时 Hub 恢复原模式。
