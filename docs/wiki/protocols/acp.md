...
- `cc-flicker` 连接由 Hub 托管的本地 MyFlickerBridge（`http://127.0.0.1:17999`），默认 `CLAUDE_OPUS_4_8`，走 **CLI gateway model discovery**（注入 `env.CLAUDE_CODE_ENABLE_GATEWAY_MODEL_DISCOVERY=1`，不用 `availableModels`/`enforceAvailableModels`），模型选择跟随 bridge `/v1/models` 动态 catalog 全量展示（非 Claude 模型带 `CLAUDE-MYFLICKER-` 前缀 id）。这样每个模型都能调 effort——白名单模式下非 Claude 模型无 `supportsEffort`、effort 选项会被隐藏。Opus/Fable 映射到 `CLAUDE_OPUS_4_8`，Sonnet/Haiku/Subagent 映射到 `CLAUDE_4_6`，WheelMaker 侧不做 effort 归一化。Hub 配置 `api_keys.flicker` 后拉起 bridge 子进程，并把该配置值原样复用为 `MYFLICKER_BRIDGE_API_KEY` 与 `cc-flicker` 的本地认证值，不额外生成随机 key；MyFlicker 的设备 token/SSO 凭据仍由 bridge 自己持有，WheelMaker 不读取或传递它们。
