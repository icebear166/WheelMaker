...
`cc-deepseek`、`cc-glm`、`cc-kimi`、`cc-qwen` 复用 `claude-agent-acp` 与 Claude Agent SDK，不在 WheelMaker 内重新实现 Anthropic Messages。它们是独立 ACP provider：使用不同的 Hub 本地 Key、Anthropic-compatible endpoint、模型白名单和 `CLAUDE_CONFIG_DIR`，但继续复用 owned ACP process、权限请求、工具调用和通用 `configOptions` 链路。
