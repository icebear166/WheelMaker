...
- 浮动态的 **RECENT SESSIONS** 标题栏和 Project 分组内容共用一张 8px 圆角卡片，内容区不再重复显示 **RECENT** 分区标题。外层会话面板提供唯一的边框与不透明背景，Recent Project 分组在该模式下移除重复的边框和圆角，避免出现大于卡片的方形底层。Plan 与 Limits 使用相同的卡片圆角、背景、标题栏和列内 padding。
