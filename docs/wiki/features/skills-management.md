> 摘要：本页维护 WheelMaker Skills Manager 2.0 的中央 Repo 最新 checkout、Scope lock、Global 链接、Project 副本、Scope/Repo 操作和迁移边界。

# Skills Management

> 设计 spec：[Skills Manager 2.0 Native Management](../../scope/2026-08-18-skills-manager-2.0-native.md)；本轮操作收敛：[Scope Update and Install](../../scope/2026-08-19-skills-scope-update-install.md)。
...
WheelMaker 迁移前曾固定依赖 skills@1.5.18。npx 只是 Node 包执行入口，核心能力来自 skills CLI；2.0 保留需要的行为语义，但不再运行 Node、npm、npx 或该 CLI。
...
当前 `cmd.skills` 路径已使用 native Git/filesystem manager：source 解析、clone/fetch/checkout、`skills/` 发现、Global 链接、Project 复制、Scope lock、扫描、安装、更新和卸载均由 WheelMaker 管理；npx 不再是运行时依赖。HubState 和 source-first 信息架构保持不变。
...
- 每个 Repo 只有一级目录，source key 的路径组件用单个下划线连接；特殊字符使用安全转义，不使用哈希目录或多层嵌套目录。
...
Refresh 不再是用户可见操作。所有依赖 Repo 内容的操作都通过同一个 latest-repo 入口：

1. 中央 clone 不存在时，直接 clone 远端默认分支的最新版本。
2. clone 已存在且工作区干净时，执行 fetch/prune，重新解析远端默认分支，并 checkout 到最新远端提交。
3. 读取当前 branch、commit 和 `skills/` 下发现的 Skill；Scope lock 只保存当前 commit，不保存远端最新 commit。

页面用 Scope lock 的当前 commit 和中央 Repo 已获取到的最新远端 SHA 比较，显示是否有更新。远端 SHA 不写入 `.skill-source-lock.json`。中央 clone dirty/untracked 时操作直接失败，不 reset、clean 或 stash。

如果本机没有中央 clone，被动扫描或重建索引不联网；检查 Repo、Update、Install 和 Install all 才会按需 clone 最新版本。中央 clone 缺失时不会回放旧 commit。
...
- 其他外部 Skill 保留。
- Update 和 Install 遇到中央 clone dirty/untracked 时直接失败，不 reset、clean 或 stash。

同名 Repo 不进入旧的冲突封锁流程；新的安装覆盖目标目录并更新该 Skill 的 source 归属。source 不再提供该 Skill 时，当前 Scope 在 Repo 更新中报告 missing 并按已确认的自动删除规则清理。

Repo 行的 Update 只处理当前 Repo；Scope 工具栏的 Update 按 lock 中的 Repo 顺序逐个处理当前 Scope。Hub 和 Project 的 Scope 独立执行。某个 Project 更新中央 clone 后，Global 链接可能立即看到新内容；其他 Project 的副本和 lock 不自动改变，直到各自执行流程。

### Install

- Install 安装 Repo 中选择的 Skill，并写入当前 Scope 的 managedSkills。
- Install all 显式安装该 Repo 当前发现的全部 Skill。
- 单个 Install 和 Repo 级 Install all 都先执行最新 Repo checkout，再把当前 Scope 已管理 Skills 同步到该 commit，最后安装所选项或全部发现的 Skill，保证一次操作不混用两个 Repo commit。
- Scope 级 Install all 对当前 Scope 的每个 Repo 分别执行上述流程；某个 Repo 更新失败时不使用旧 checkout 继续安装，其他 Repo 继续执行并在 operation 中汇总结果。
...
- Scope 工具栏提供当前 Scope 的 Update 和 Install all；Repo/source 标题行提供该 Repo 的 Update、Install all 和删除。
- 不提供可见的 Refresh 按钮；fetch 和 checkout 最新版本由 Update/Install 内部完成。
- Skill 行的操作列只提供下载和卸载，不提供 Skill 级更新；Skill 名称仍可打开详情。
...
`refreshRepo` 仍可作为旧客户端的传输兼容 action，但不再由当前 UI 暴露；当前用户操作统一走 Update/Install 的 latest-repo 流程。普通 scan 只读；会写 source lock、中央 clone、Global 链接或 Project 副本的动作进入 Hub 单一异步 operation。Registry protocol version 保持不变。

## 状态、失败与安全边界

- Scope lock 写入采用同目录临时文件、关闭后原子替换，并检查磁盘版本，避免并发写覆盖；不创建 `.skill-source-lock.json.lock` sidecar。
