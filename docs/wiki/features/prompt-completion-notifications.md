...
- 状态差异由图标/颜色承担：PWA 正文加符号前缀（✓ / ✗ / ■），APK 用 `setColor` 染色，exe 自绘色点。
...
替换键统一为 `projectId:sessionId`：同一会话最多保留一条通知，新完成事件替换旧通知内容并重新提醒。PWA 用 `tag` + `renotify`，APK 用固定通知 ID 原地更新，exe 按会话键归并自绘小窗并重置自动消失计时。
...
- exe：Go 小窗点击后还原并聚焦主窗口，Eval 派发 `wheelmaker:desktop-notification-click` 事件，web 运行时入口完成跳转。
...
- **exe**：Go 宿主自绘 Win32 置顶小窗（`WS_EX_TOPMOST | WS_EX_TOOLWINDOW | WS_EX_NOACTIVATE`），主窗口所在显示器右下角堆叠、数秒自动消失；不经过 WebView2 通知管道（其权限请求在无宿主处理时被静默拒绝，且 toast 点击不回传），因此不需要系统通知授权，desktop provider 权限恒为 granted。

来源：`docs/scope/2026-08-17-prompt-completion-notifications.md`。
