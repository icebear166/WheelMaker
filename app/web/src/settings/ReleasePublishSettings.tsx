...
export function ReleasePublishSettings({hubIds, start, query}: {
  ...
  return <div className="settings-metadata-card">
    ...
    <div className="settings-form-grid">
      ...
      <label>Server Hub<select value={settings.serverHubId} onChange={event => update({serverHubId: event.target.value, autoPull: event.target.value ? settings.autoPull : false})}><option value="">No automatic apply</option>{hubIds.filter(id => id !== settings.publisherHubId).map(id => <option key={id} value={id}>{id}</option>)}</select></label>
