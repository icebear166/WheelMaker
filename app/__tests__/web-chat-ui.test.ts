...
describe('web chat integration', () => {
  ...
  test('defines registry session protocol and uses real chat UI instead of placeholder sessions', () => {
    ...
    expect(mainTsx).toContain("import { formatChatContextUsage, splitChatComposerStatusOptions } from '../chat/session/chatComposerStatus';");
    ...
    expect(mainTsx).toContain('{chatConfigStatus.showOverflowToggle ? (');
    ...
    expect(mainTsx).toContain("className={`codicon ${chatConfigOverflowOpen ? 'codicon-chevron-up' : 'codicon-chevron-down'}`}");
    ...
    expect(mainTsx).toContain("title={chatConfigOverflowOpen ? 'Hide config options' : 'Show config options'}");
