...
describe('web chat integration', () => {
  ...
  test('chat breadcrumb title uses the selected session project', () => {
    ...
    expect(mainTsx).toContain('className="breadcrumb-title chat-breadcrumb-title chat-mobile-title-button"');
    expect(mainTsx).toContain('onClick={handleMobileBreadcrumbProjectClick}');
  ...
  test('chat drawer header keeps tools left and hub browser right', () => {
    ...
    expect(mobileChatHeaderSearchOpenBlock).not.toContain('+ 62px');
    ...
    expect(mobileSearchWrapBlock).toContain('height: 38px;');
    expect(mobileSearchWrapBlock).toContain('display: block;');
    expect(stylesCss).toContain('.mobile-chat-drawer-header.search-open .chat-header-search-control.open {');
    const mobileSearchOpenControlBlock = stylesCss.match(/\.mobile-chat-drawer-header\.search-open \.chat-header-search-control\.open \{[\s\S]*?\n\}/)?.[0] ?? '';
    expect(mobileSearchOpenControlBlock).toContain('height: 28px;');
    expect(mobileSearchOpenControlBlock).toContain('position: absolute;');
    expect(mobileSearchOpenControlBlock).toContain('top: 0;');
    ...
    expect(mobileSearchStatusBlock).toContain('position: absolute;');
    expect(mobileSearchStatusBlock).toContain('display: flex;');
    expect(mobileSearchStatusBlock).toContain('font-size: 9px;');
    expect(mobileSearchStatusBlock).toContain('height: 9px;');
    ...
    expect(mobileSearchStatusBlock).toContain('right: 3px;');
    expect(mobileSearchStatusBlock).toContain('bottom: -2px;');
    expect(mobileSearchStatusBlock).toContain('padding: 0;');
  ...
  test('wires speech input settings and composer voice controls through split modules', () => {
    ...
    expect(settingsRootTsx).toContain('speechSettings.enabled ? (');
  ...
  test('mobile chat drawer uses a cross-project project session sheet', () => {
    ...
    const appearanceSettingsIndex = settingsRootTsx.indexOf("renderSettingsSection('Appearance'");
    const chatSettingsIndex = settingsRootTsx.indexOf("renderSettingsSection('Chat'");
    const codeDisplaySettingsIndex = settingsRootTsx.indexOf("renderSettingsSection('Code Display'");
  ...
  test('wide layout uses a project session rail instead of the header project picker', () => {
    ...
    const chatTitleProjectMenuStart = mainTsx.indexOf('const chatTitleProjectMenu = isWide && chatTitleProjectMenuOpen ? (');
    ...
    expect(mobileTitleBlock).toContain('className="breadcrumb-title chat-breadcrumb-title chat-mobile-title-button"');
    expect(mobileTitleBlock).toContain('onClick={handleMobileBreadcrumbProjectClick}');
    expect(mobileTitleBlock).not.toContain('chat-title-prompt-icon-button');
    expect(mobileTitleBlock).not.toContain('chat-title-project-button');
  ...
  test('chat title bar uses breadcrumb context and exposes preview toggle', () => {
    ...
    expect(mainTsx).toContain('const chatTitleProjectMenu = isWide && chatTitleProjectMenuOpen ? (');
    expect(mainTsx).toContain('const chatTitlePromptMenu = isWide && chatTitlePromptMenuOpen && chatTitlePromptMenuAvailable ? (');
    ...
    expect(stylesCss).toContain('.chat-mobile-title-button {');
    expect(stylesCss).toContain('.chat-sidebar-toggle,');
    ...
    const mobileTitleBlock = cssRuleBlock(stylesCss, '.chat-mobile-title-button');
    expect(mobileTitleBlock).toContain('width: 100%;');
    expect(mobileTitleBlock).toContain('border: 0;');
    expect(mobileTitleBlock).toContain('background: transparent;');
    expect(mobileTitleBlock).toContain('cursor: pointer;');
