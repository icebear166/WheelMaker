...
describe('chat edge surface geometry', () => {
  test('clips transparent desktop edge tails out of pointer hit testing until reveal', () => {
    ...
    expect(stylesCss).toMatch(
      /\.chat-recent-sessions-surface\.desktop:is\(:hover, :focus-within\),[\s\S]*\.chat-plan-surface\.desktop:is\(\.chat-edge-surface-hover-revealed, :focus-within\),[\s\S]*\.chat-function-surface\.desktop:is\(\.chat-edge-surface-hover-revealed, :focus-within\) \{[\s\S]*--chat-edge-hidden-alpha: 100%;[\s\S]*clip-path: none;[\s\S]*\}/,
    );
